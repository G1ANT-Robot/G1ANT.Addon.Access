using G1ANT.Addon.Access.Api.Access;
using G1ANT.Addon.Access.Forms;
using G1ANT.Addon.Access.Models.Access;
using G1ANT.Addon.Access.Models.Access.AccessObjects;
using G1ANT.Addon.Access.Models.Access.Application;
using G1ANT.Addon.Access.Models.Access.Dao;
using G1ANT.Addon.Access.Models.Access.Dao.Containers;
using G1ANT.Addon.Access.Models.Access.Dao.Documents;
using G1ANT.Addon.Access.Models.Access.Dao.Fields;
using G1ANT.Addon.Access.Models.Access.Dao.Parameters;
using G1ANT.Addon.Access.Models.Access.Dao.Properties;
using G1ANT.Addon.Access.Models.Access.Dao.QueryDefs;
using G1ANT.Addon.Access.Models.Access.Data;
using G1ANT.Addon.Access.Models.Access.Forms.Recordsets;
using G1ANT.Addon.Access.Models.Access.Modules;
using G1ANT.Addon.Access.Models.Access.Printers;
using G1ANT.Addon.Access.Models.Access.Resources;
using G1ANT.Addon.Access.Models.Access.VBE;
...
namespace G1ANT.Addon.Access.Controllers
{
    public class AccessUIControlsTreeController
    {
        ...
        private string GetPathToNode(TreeNode node)
        {
            ...
            switch (node.Tag)
            {
                case RotApplicationModel rotApplicationModel:
                    return null;

                case AccessControlModel controlModel:
                    return GetControlPath(node);

                case AccessFormModel formModel:
                    if (node.Parent.Text == FormLabel && node.Parent.Parent?.Tag is AccessControlModel)
                        return GetControlPath(node.Parent.Parent);
                    return formModel.Name;

                case INameModel nameModel:
                    return nameModel.Name;
            }
        ...
        {
            var path = GetPathToNode(node);
            InsertPathIntoScript(path);
        }

        private void InsertPathIntoScript(string path)
        {
        ...
        private string GetControlPath(TreeNode node)
        {
            var path = "";
            while (node.Parent != null)
            {
                ...
                if (name == null)
                    break;
                path = $"{name}{ControlPathModel.PathSeparator}{path}";
                node = node.Parent;
            }
            return ControlPathModel.PathSeparator + path;
        }
