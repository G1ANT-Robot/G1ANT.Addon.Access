...
{
    [Command(Name = "access.deselectcontrolitem", Tooltip = "Mark item of Access control selected by path as unselected")]
    public class AccessDeselectItemCommand : Command
    {
        ...
        public AccessDeselectItemCommand(AbstractScripter scripter) : base(scripter)
        { }

        public void Execute(Arguments arguments)
        {
            var control = AccessManager.CurrentAccess.GetControlByPath(arguments.Path.Value);
            control.SetItemSelected(arguments.Index.Value, false);
        }
    }
}
